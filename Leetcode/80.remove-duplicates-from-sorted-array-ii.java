...
class Solution {
    ...
    // 📝 Solved by me works fine (5ms execution ) Not optimal
    // public static int removeDuplicates(int[] nums) {
    //     Map<Integer, Integer> occu = new HashMap<>();
    //     int index = 0;
        
    //     for (int i = 0; i < nums.length; i++) {
    //         occu.merge(nums[i], 1, Integer::sum);

    //         if(occu.get(nums[i]) <= 2){
    //             nums[index++] = nums[i];
    //         } 
    //     }
    ...
    // ⚠️ Copied the solution, optima without hashMap (Runtime 1ms)
    public static int removeDuplicates(int[] nums) {
        int k = 2;
